"""Noise-sensitivity (causal) experiment."""
from ..config import CFG
from ..factories import make_noise_sensitivity_model
from ..utils import out_path, save_json, already_done, compute_metrics
from ..evaluation import run_evaluation
from .helpers import run_slug, run_config


def experiment_noise_sensitivity(datasets, num_workers, force, device):
    print("\n=== Phase 1: Noise-Sensitivity Channel Selection ===")
    ...
    print(f"  backbone={CFG.backbone}  feat={feat_layer}"
          f"  pf={CFG.percent_features}  ws={CFG.window_size}  noise_std={CFG.noise_std}")
    summary_file = f"phase1_noise_sensitivity_{slug}_summary.json"
    results = {}

    for ds in datasets:
        model = make_noise_sensitivity_model(device, feature_layer=feat_layer).eval()
        csv_name = f"noise_sensitivity_{slug}_{ds}.csv"
